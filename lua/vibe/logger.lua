---@brief Centralized logger for the Vibe Neovim integration.
...
---@param plugin_config VibeConfig The configuration table (e.g., from vibe.init.state.config).
function M.setup(plugin_config)
  ...
  if conf and conf.log_level and level_values[conf.log_level] then
    ...
    vim.notify(
      "Vibe Logger: Invalid or missing log_level in configuration (received: "
...
local function log(level, component, message_parts)
  ...
  vim.schedule(function()
    if level == M.levels.ERROR then
      vim.notify(prefix .. " " .. message, vim.log.levels.ERROR, { title = "Vibe Error" })
    elseif level == M.levels.WARN then
      vim.notify(prefix .. " " .. message, vim.log.levels.WARN, { title = "Vibe Warning" })
    else
...
---@param level_name VibeLogLevel The level name ("error", "warn", "info", "debug", "trace")
