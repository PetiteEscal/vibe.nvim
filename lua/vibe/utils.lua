---Shared utility functions for vibe.nvim
---@module 'vibe.utils'
...
---providers can spawn Vibe directly (without a shell) while preserving quoted
---arguments such as `--workdir='some path'`. Spawning without a shell also
---avoids glob expansion of bracketed tokens.
...
---Vibe directly (no shell) while still preserving quoted arguments and the
---documented `terminal_cmd = "~/.local/bin/vibe"` install path.
---Globbing and variable expansion are deliberately NOT performed.
---@param cmd string
---@return string[] argv
...
---Make a path relative to the current working directory when possible, so the
---`@path` mention Vibe receives is short and readable. Falls back to the
---absolute path when the file is outside the cwd.
---@param path string Absolute or relative path.
---@return string formatted The path to mention.
